...
N = 16; % liczba węzłów interpolacji (z przedziału [12, 20])
    ...
    coeff_uniform = V_uniform \ y_uniform'; % rozwiązywanie układu równań liniowych
    ...
    coeff_chebyshev = V_chebyshev \ y_chebyshev'; % współczynniki wielomianu
    coeff_chebyshev = coeff_chebyshev(end:-1:1);  % dostosowanie do polyval
    ...
    xlabel('x');
    ylabel('y');
    title('Interpolacja – węzły Czebyszewa II rodzaju');
...
function x = get_chebyshev_nodes(N)
    % Węzły Czebyszewa II rodzaju wyznaczone w przedziale [-1, 1]
    k = 0:N-1;
    x = cos(pi * k / (N - 1));
end

function V = get_vandermonde_matrix(x)
    % Buduje macierz Vandermonde’a dla wektora węzłów x
    N = length(x);
    V = zeros(N);
    for i = 1:N
        V(:, i) = x.^(i-1);
