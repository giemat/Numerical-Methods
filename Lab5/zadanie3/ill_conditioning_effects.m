...
%       zaburzone wartości funkcji kwadratowej 
    ...
    for index = 1:num_points
        ...
        V = get_vandermonde_matrix(interpolation_nodes);  % TODO: użyj funkcji

        condition_numbers(index) = cond(V);               % TODO: oblicz cond(V)
    end
    ...
    nexttile;
    semilogy(matrix_sizes, condition_numbers, 'LineWidth', 1.2);  % TODO: narysuj semilogy
    ...
    for index = 1:num_points
        ...
        reference_coefficients = [0; 0; a2; zeros(size_n - 3, 1)];  % TODO

        computed_coefficients = V \ b_exact;               % TODO: rozwiąż układ

        interpolation_error_exact(index) = ...
            max(abs(computed_coefficients - reference_coefficients));  % TODO
    end

    % Wykres 2
    nexttile;
    plot(matrix_sizes, interpolation_error_exact, 'LineWidth', 1.2);  % TODO
    xlabel('Rozmiar macierzy N');
    ...
    for index = 1:num_points
        ...
        computed_coefficients = V \ b_perturbed;           % TODO
    ...
    % Wykres 3
    nexttile;
    semilogy(matrix_sizes, interpolation_error_perturbed, 'LineWidth', 1.2);  % TODO
    xlabel('Rozmiar macierzy N');
    ylabel('Błąd max |a - a_{ref}|');
